...
Чисто технически ничего не запрещает вам везде использовать строковый подстановочный символ `%s`. `writeln`, который в большинстве случаев сможет корректно привести тот или иной тип к строковому и вывести его. Однако в случаях, когда вам нужно, чтобы, к примеру, тип с плавающей точкой был выведен до определённого количества знаков после точки, тогда рекомендуется указывать его тип в явном виде.
...
Строго говоря, строка является частным случаем массива. Упрощено массив можно представить следующим образом:
...
aa["one"] = "Piter";
```
Изменит значение для ключа `one` на имя `Piter`. Если вы попытаетесь обратиться к несуществующему ключу, будет выкинуто исключение:
```
...
...
`&&` логическое `и`. Если все условия истины, то будет возвращено `true`, если нет, то `false`
`>`, `<`, `>=`, `<=`. Операторы позволяют оценивать чистовые значения и возвращать `true` или `false` в зависимости от результата проверки.
...
{
  // Данные методы должны быть реализованы во всех производных классах
  void talk();
  void walk();
}
...
void main()
{
  ManClass manClass = new ManClass("David"); // передаем данные в конструктор класса
  manClass.talk(); // вызываем метод класса
  manClass.walk("David"); // передаем данные в метод напрямую

}

interface IHuman // в интерфейсе созданы два метода
{
  // Данные методы должны быть реализованы во всех производных классах
  void talk();
  void walk(string manname); // если реализованный метод должен принимать данные, то и метод интерфейса должен их принимать
}
...
class ManClass : IHuman
{
  ...
  void talk()
  {
    writefln("%s can talk", name); // используем данные из конструктора
  }

  void walk(string manname) // принимаем данные, переданные в метод
  {
...
class WomanClass : IHuman
{
  ...
  void walk(string womanname) // тоже принимает данные, т.к. в реализации 
  {
    // пока не реализовано  
  }
